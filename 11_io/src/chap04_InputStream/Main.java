 ...
 *                    (1바이트를 int로 변환하면 0~255 값으로변환되는데 이렇게 처리하면 -1(EOF)과 반환하는 값의 차이를 만들 수 있습니다.)
 *                    (1바이트를 byte 그대로 변환하면 -128 ~ 127 값으로 반환하는데 이러면 -1(EOF)과 반환하는 값이 같아 구분이 어렵습니다.)
 *    2) int read(byte[] b) : 전달한 바이트 배열만큼 읽어서 배열에 저장 (읽은 데이터는 byte[] b에 저장됩니다)
 *                            실제로 읽은 바이트의 수를 반환합니다.
 *    3) int read(byte[] b, int off, int len) : 전달한 배열의 일부만 사용해서 읽어 저장(인덱스 off 부터 len까지)
 *                                              실제로 읽은 바이트 수를 반환합니다.
...
//            List<Byte> list = new ArrayList<Byte>();
    ...
    //----- 저장되어 있는 순서대로 읽습니다.
        int x = din.readInt();          //---- writeInt() 로 생성한 값
        double y = din.readDouble();    //---- writeDouble() 로 생성한 값
