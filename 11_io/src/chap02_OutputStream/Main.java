 ...
 *    4) PrintStream
 * 4. 주요 메소드
 *    1) write(int b) : 1바이트를 출력합니다. (4바이트중 하위 1바이트만 사용합니다.)
 *    2) write(byte[] b) : 바이트 배열 전체를 출력합니다.
 *    3) write(byte[] b, int off, int len) : 바이트 배열 일부를 출력합니다.(인덱스 off부터 len개 만큼)
...
public class Main {
    public static void fileCreate(){
        ...
        OutputStream os = null;  // try, catch, finally 블록에서 모두 접근 가능한 scope
...
//          os.write('A');     // 1바이트만 출력(정상 출력)
            os.write('홍');  // 1바이트만 출력
                                // '홍'의 유니코드 값은 U+D64D(1101 0110 0100 1101)
                                // 유니코드값 중 마지막 1바이트만 출력 (실제 출력은 0100 1101)
                                // 0100 1101 == 77 이므로 코드 값 77인 문자
            byte[] bytes = "Hello World".getBytes();
            os.write(bytes,0,5);                // 인덱스 0 부터 5개 요소만 출력합니다.
        } catch (FileNotFoundException e) {
            System.out.println(file.getPath() + " 파일을 찾을 수 없습니다.");  // 저장된 디렉터리와 파일
        } catch (IOException e){
    ...
    public static void fileAppend(){
        ...
        try {
            os = new FileOutputStream(file, true); // true를 추가하면 파일 추가 모드가 됩니다. 기존 내용의 뒤에 덧붙인다
            os.write('!');
    ...
    public static void bufferedOutputStream(){
         ...
         * 3. 내부 버퍼에 쌓인 출력 데이터는 내부 버퍼가 가득차거나 /flush() 호출 /close() 호출 시 한 번에 출력됩니다.
         */
        File file = new File("D:/storage/test.dat");
    ...
    public static void dataOutputStream(){
         ...
         *
         * 1. 기본타입(Primitive Type) 데이터와 문자열(String)을 이진(binary)형태로 출력할 수 있도록 해주는
         * 2. 주요 메소드
