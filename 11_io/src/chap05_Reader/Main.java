 ...
 *                               실제로 읽은 문자 수 반환
 *                               (파일의 끝에 도달하면 EOF(-1) 반환)
 *     3) int read(char[] cbuf, int off, int len) : 읽은 내용을 문자 배령 일부에 저장(인덱스 off부터 len개까지)
 *                                                  실제로 읽은 문자 수 반환
 *  *                                               (파일의 끝에 도달하면 EOF(-1) 반환)
 */
public class Main {
    ...
    public static void bufferedReader(){
         ...
         *
         * 1. 문자 기반의 입력 스트림에 버퍼링 기능을 제공하는 보조 스트림 클래스입니다.
  ...
  //      try(InputStreamReader in = new InputStreamReader(new FileInputStream(file))){ 한글이 안 깨지지만, 성능이 떨어집니다
...
//                sb.append((char)c);
//            }
            // BufferReader를 사용하는 경우 다음과 같이 읽기를 추천합니다.
            String line =null;
            while( (line = in.readLine()) != null ) {
                sb.append(line).append("\n");  // 개행 문자까지 동일하게 처리하려면 sb.append(line).append("\n")
            }
            System.out.println(sb.toString());
