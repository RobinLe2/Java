 ...
 * Map 인터페이스
 *
 * 1. JCF 에서 키(Key)와 값(Value)의 쌍으로 구성된 데이터를 저장하는 구조입니다.
 * 2. 키(Key)는 값(Value)을 얻기위한 식별자로 사용되기 때문에 중복이 불가능합니다.
 * 3. 값(Value)은 Map이 저장하려는 데이터이면 중복 값을 허용합니다.
 * 4. 키(Key)와 값(Value)을 합쳐서 하나의 Entry 라고 부릅니다.
...
public class Main {
    public static void mutable(){
        // mutalbe : 데이터의 추가, 수정, 삭제가 가능한 Map
