package chap06_constructor;
  ...
  *  2. 오직 객체(인스턴스) 생성할 때만 호출 할 수 있습니다.
  *  3. 객체 생성 키워드 new 다음에 생성자를 호출합니다.
  *  4. 주요 특징
  *     1) 생성자의 이름은 클래스의 이름과 반드시 같아야 합니다.
  *     2) 일반 메소드와 달리 반환 타입이 존재하지 않습니다.
 ...
 *  1. 매개변수가 없는 생성자입니다.
 *  2. 자동 생성
 *     클래스에 생성자를 하나도 정의하지 않으면, 자바 컴파일러는 매개변수가 없고 본문이 비어있는 디폴트 생성자를 자동으로생성합니다.
 *     예시 형태) Car() { }
 *  3. 직접 정의
 *     클래스에 생성자를 하나 이상 정의하면, 디폴트 생성자는 더 이상 자동으로 생성되지 않습니다.
 */
public class Car {
  ...
  Car(String model, int price){
    this.model = model;     // this(model); -> 일반 생성자 1을 호출하는 코드입니다. 생성자 내부에서 다른 생성자를 호출할 때 this() 문법을 이용합니다.
    this.price = price;
  }
