...
1. 프로그램 실행 중 발생할 수 있는 비정상 적인 상황(예외 상황)에 대해 적절히 대응하는 것을 의미합니다.
2. 발생한 예외로 인해서 프로그램이 비정상적으로 종료되는 것으 막을 수 있습니다.
3. 발생한 예외를 분석하여 예외의 원인을 파악하고 복구를 시도할 수 있습니다.
4. 기본 구문
    1) try 블록 : 예외 발생 가능성이 있는 코드를 작성하는 블록
    2) catch 블록 : 예외가 발생했을 때 실행 할 코드를 작성합니다.
...
2. 발생한 예외를 받아주는 첫 catch 블록이 예외를 처리합니다.
...
public class Main {
    ...
    public static void tryCatch() {
        ...
        // 예외 발생 가능성이 있는 코드 (try 블록)
        try {
            String input = JOptionPane.showInputDialog("정수를 입력하세요.");
            int number = Integer.parseInt(input);
            System.out.println(number);
        }
        // 예외 발생 시 동작할 코드(catch) 블록 - catch() 여기에서는 꼭 매개변수가 필요.
    ...
    public static void javaLangException() {
        try {
            ...
            System.out.println(a * b);
            System.out.println(a / b);
        } catch (Exception e) {  //-------------- 거의 모든 예외를 받을 수 있는 java.Exception 클래스
            // 1. 예외 클래스 확인하기
