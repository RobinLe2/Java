 ...
 * java.net.URLEncoder
 *
 * 1. 문자열을 URL 인코딩 형식(application/x-www.form-urlencoded)
 * 2. 특수 문자나 한글 같은 비ASCII문자가 포함될 때 사용합니다.
 * 3. 인코딩 규칙
 *   1) 알파벳(A-Z, a-z), 숫자(0-9), 일부 특수 문자(. - _ *)는 그대로 유지합니다.
 *   2) 공백 문자는 + 로 변환합니다.
 *   3) 나머지 문자들은 인코딩된 바이트 값을 %xx(16진수) 형식으로 변환합니다.
 * 4. 잘못된 인코딩 규칙을 사용하면 java.io.UnsupportedEncodingException 예외가 발생합니다.(checked)
 */
public class Main {
    /**
    ...
    /**
     * UTF-8로 인코딩 된 문자열을 전달 받아서 디코딩한 결과를 반환하는 메소드 (복호화)
     * @param str
     * @return
